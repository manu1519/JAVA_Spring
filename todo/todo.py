from flask import(redirect,Blueprint,flash,g,render_template,request,url_for,session)
...
def index():
    db,c = get_db()
    c.execute(
        'select t.id, t.description, u.username, t.completed, t.created_at from todo t JOIN user u on t.created_by = u.id where t.created_by = %s order by created_by desc', (g.user['id'],)
    )
    todos = c.fetchall()
...
def update(id):
    ...
    if request.method == 'POST':
        description = request.form['description']
        completed = True if request.form.get('completed') == 'on' else False # Solo si se usa una checkbox
        error = None
